...
def main():
    ...
    ap.add_argument("--calib-format", type=str, default="Numpy", choices=["Numpy", "Binary", "NumpyObject", "Image"], help="Formato de datos de calibración para Pulsar2")
    ...
    ap.add_argument("--tar-name", type=str, default="calib_kws.tar", help="Nombre del archivo TAR de salida (por defecto: calib_kws.tar)")
    args = ap.parse_args()
    ...
    if files:
        ...
        with tarfile.open(tar_path, "w") as tar:
            for fpath in files:
                tar.add(fpath, arcname=fpath.name)  # sin subcarpetas
